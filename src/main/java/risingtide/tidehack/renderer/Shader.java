...
public class Shader {
    ...
    public Shader(String vertPath, String fragPath) {
        ...
        if (vertError != null) {
            TideHack.LOG.error("Failed to compile vertex shader (" + vertPath + "): " + vertError);
        ...
        if (fragError != null) {
            TideHack.LOG.error("Failed to compile fragment shader (" + fragPath + "): " + fragError);
        ...
        if (programError != null) {
            TideHack.LOG.error("Failed to link program: " + programError);
    ...
    private String read(String path) {
        try {
            return IOUtils.toString(Utils.mc.getResourceManager().getResource(new Identifier("meteor-client", "shaders/" + path)).getInputStream(), StandardCharsets.UTF_8);
        } catch (IOException e) {
            e.printStackTrace();
            return "";
        }
