...
import risingtide.tidehack.events.meteor.ActiveModulesChangedEvent;
...
import risingtide.tidehack.systems.modules.combat.*;
import risingtide.tidehack.systems.modules.misc.*;
import risingtide.tidehack.systems.modules.misc.swarm.Swarm;
import risingtide.tidehack.systems.modules.movement.*;
import risingtide.tidehack.systems.modules.movement.elytrafly.ElytraFly;
import risingtide.tidehack.systems.modules.movement.speed.Speed;
import risingtide.tidehack.systems.modules.player.*;
import risingtide.tidehack.systems.modules.render.*;
import risingtide.tidehack.systems.modules.render.hud.HUD;
import risingtide.tidehack.systems.modules.render.search.Search;
import risingtide.tidehack.systems.modules.world.*;
import risingtide.tidehack.systems.modules.world.Timer;
import risingtide.tidehack.utils.Utils;
import risingtide.tidehack.utils.misc.input.Input;
...
public class Modules extends System<Modules> {
    ...
    void addActive(Module module) {
        synchronized (active) {
            if (!active.contains(module)) {
                active.add(module);
                TideHack.EVENT_BUS.post(ActiveModulesChangedEvent.get());
            }
        }
    }
    ...
    private boolean onBinding(boolean isKey, int value) {
        if (moduleToBind != null && moduleToBind.keybind.canBindTo(isKey, value)) {
            ...
            TideHack.EVENT_BUS.post(ModuleBindChangedEvent.get(moduleToBind));
            moduleToBind = null;
            return true;
        }
    ...
    private void onGameJoined(GameJoinedEvent event) {
        synchronized (active) {
            for (Module module : modules) {
                if (module.isActive()) {
                    TideHack.EVENT_BUS.subscribe(module);
                    module.onActivate();
                }
            }
        }
    ...
    private void onGameLeft(GameLeftEvent event) {
        synchronized (active) {
            for (Module module : modules) {
                if (module.isActive()) {
                    TideHack.EVENT_BUS.unsubscribe(module);
                    module.onDeactivate();
                }
            }
        }
