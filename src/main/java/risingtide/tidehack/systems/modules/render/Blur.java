...
import risingtide.tidehack.events.render.RenderAfterWorldEvent;
import risingtide.tidehack.gui.WidgetScreen;
import risingtide.tidehack.renderer.Framebuffer;
...
public class Blur extends Module {
    ...
    public Blur() {
        ...
        TideHack.EVENT_BUS.subscribe(new ConsumerListener<>(WindowResizedEvent.class, event -> {
            if (fbo1 != null) {
                fbo1.resize();
                fbo2.resize();
            }
        ...
        TideHack.EVENT_BUS.subscribe(new ConsumerListener<>(RenderAfterWorldEvent.class, event -> onRenderAfterWorld()));
    }
