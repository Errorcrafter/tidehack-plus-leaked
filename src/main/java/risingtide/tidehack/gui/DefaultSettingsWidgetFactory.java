...
import risingtide.tidehack.gui.screens.settings.*;
import risingtide.tidehack.gui.screens.settings.*;
import risingtide.tidehack.gui.utils.SettingsWidgetFactory;
import risingtide.tidehack.gui.widgets.*;
import risingtide.tidehack.gui.widgets.containers.*;
import risingtide.tidehack.gui.widgets.*;
import risingtide.tidehack.gui.widgets.input.WDoubleEdit;
...
public class DefaultSettingsWidgetFactory implements SettingsWidgetFactory {
    ...
    private void genericW(WTable table, GenericSetting<?> setting) {
        ...
        edit.action = () -> Utils.mc.openScreen(setting.get().createScreen(theme));
    ...
    private void colorW(WTable table, ColorSetting setting) {
        ...
        edit.action = () -> Utils.mc.openScreen(new ColorSettingScreen(theme, setting));
    ...
    private void blockListW(WTable table, BlockListSetting setting) {
        selectW(table, setting, () -> Utils.mc.openScreen(new BlockListSettingScreen(theme, setting)));
    }
    ...
    private void itemListW(WTable table, ItemListSetting setting) {
        selectW(table, setting, () -> Utils.mc.openScreen(new ItemListSettingScreen(theme, setting)));
    }

    private void entityTypeListW(WTable table, EntityTypeListSetting setting) {
        selectW(table, setting, () -> Utils.mc.openScreen(new EntityTypeListSettingScreen(theme, setting)));
    }

    private void enchantmentListW(WTable table, EnchantmentListSetting setting) {
        selectW(table, setting, () -> Utils.mc.openScreen(new EnchantmentListSettingScreen(theme, setting)));
    }

    private void moduleListW(WTable table, ModuleListSetting setting) {
        selectW(table, setting, () -> Utils.mc.openScreen(new ModuleListSettingScreen(theme, setting)));
    }

    private void packetListW(WTable table, PacketListSetting setting) {
        selectW(table, setting, () -> Utils.mc.openScreen(new PacketBoolSettingScreen(theme, setting)));
    }

    private void particleTypeListW(WTable table, ParticleTypeListSetting setting) {
        selectW(table, setting, () -> Utils.mc.openScreen(new ParticleTypeListSettingScreen(theme, setting)));
    }

    private void soundEventListW(WTable table, SoundEventListSetting setting) {
        selectW(table, setting, () -> Utils.mc.openScreen(new SoundEventListSettingScreen(theme, setting)));
    }

    private void statusEffectAmplifierMapW(WTable table, StatusEffectAmplifierMapSetting setting) {
        selectW(table, setting, () -> Utils.mc.openScreen(new StatusEffectAmplifierMapSettingScreen(theme, setting)));
    }

    private void statusEffectListW(WTable table, StatusEffectListSetting setting) {
        selectW(table, setting, () -> Utils.mc.openScreen(new StatusEffectListSettingScreen(theme, setting)));
    }

    private void storageBlockListW(WTable table, StorageBlockListSetting setting) {
        selectW(table, setting, () -> Utils.mc.openScreen(new StorageBlockListSettingScreen(theme, setting)));
    }
    ...
    private void blockDataW(WTable table, BlockDataSetting<?> setting) {
        ...
        button.action = () -> Utils.mc.openScreen(new BlockDataSettingScreen(theme, setting));
