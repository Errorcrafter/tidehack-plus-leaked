...
import risingtide.tidehack.events.render.RenderBossBarEvent;
...
public class BossBarHudMixin {
    ...
    public Iterator<ClientBossBar> onRender(Collection<ClientBossBar> collection) {
        RenderBossBarEvent.BossIterator event = TideHack.EVENT_BUS.post(RenderBossBarEvent.BossIterator.get(collection.iterator()));
    ...
    public Text onAsFormattedString(ClientBossBar clientBossBar) {
        RenderBossBarEvent.BossText event = TideHack.EVENT_BUS.post(RenderBossBarEvent.BossText.get(clientBossBar, clientBossBar.getName()));
        return event.name;
    }

    @ModifyConstant(method = "render", constant = @Constant(intValue = 9, ordinal = 1))
    public int modifySpacingConstant(int j) {
        RenderBossBarEvent.BossSpacing event = TideHack.EVENT_BUS.post(RenderBossBarEvent.BossSpacing.get(j));
